...
static int  ReadPreference(char *fname, int argc, FNType *path);
...
/* taking care of preferences in preferences file */
	...
	strcpy(home_dir, "C:\\childes\\clan");
	if (chdir(home_dir)) {
...
// strcpy(templineC, ctime((time_t *)&versionTime));
	strcpy(templineC2, "http://childes.psy.cmu.edu/clan/CLAN_VERSION.txt");
/*
	DH  = NewHandle(0);
	if (MemError()) {
		WriteCedPreference();
...
//				my_FSPathMakeRef(gFile, &tRef); 
			...
			strcpy(templineC, "http://childes.psy.cmu.edu/clan/clan.dmg");
