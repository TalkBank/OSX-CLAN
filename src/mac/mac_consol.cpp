...
static AEEventHandlerUPP hOpenFinderDoc;
static AEEventHandlerUPP hAEQuitCed;
static AEEventHandlerUPP hOpenFilePos;
...
void ControlAllControls(int on, short id) {
	...
	if (id == -1) {
		...
		EnableMenuItem(EditMenu, selectall_id);
 		...
 		DisableMenuItem(EditMenu, resetAllOptions_id);
	...
	if (id == 501 || id == 506) {
		...
		EnableMenuItem(EditMenu, selectall_id);
 		...
 		EnableMenuItem(EditMenu, resetAllOptions_id);
...
const char *sendMessage(const char *mess) {
	AEDesc pDesc;
...
static struct SelectedFilesList *addToSelectedFiles(struct SelectedFilesList *root, FNType *fname) {
...
void InstallAEProcs(void) {
	short err = 0;

	hOpenFinderDoc = NewAEEventHandlerUPP((AEEventHandlerProcPtr)OpenFinderDoc);
	err = AEInstallEventHandler(kCoreEventClass, kAEOpenDocuments, hOpenFinderDoc, 0, false);
	hAEQuitCed = NewAEEventHandlerUPP((AEEventHandlerProcPtr)AEQuitCed);
	err = AEInstallEventHandler(kCoreEventClass, kAEQuitApplication, hAEQuitCed, 0, false);
...
void RemoveAEProcs(void) {
	short err = 0;

	err = AERemoveEventHandler(kCoreEventClass, kAEOpenDocuments, hOpenFinderDoc, false);
	err = AERemoveEventHandler(kCoreEventClass, kAEQuitApplication, hAEQuitCed, false);
