...
int soundwindow(int TurnOn) {
	...
	if (GetFontNumber(fontName, &fID)) {
		tf.FName = fID;
		tf.FSize = 15;	
		tf.CharSet = my_FontToScript(tf.FName, 0);
		tf.Encod = tf.CharSet;
		tf.FHeight = GetFontHeight(&tf, NULL, global_df->wind);
		if ((rows * tf.FHeight) < global_df->SoundWin->winPixelSize) {
			do {
				tf.FSize++;	
				tf.FHeight = GetFontHeight(&tf, NULL, global_df->wind);
			} while ((rows * tf.FHeight) <= global_df->SoundWin->winPixelSize) ;
			tf.FSize--;
		} else if ((rows * tf.FHeight) > global_df->SoundWin->winPixelSize) {
			do {
				tf.FSize--;	
				tf.FHeight = GetFontHeight(&tf, NULL, global_df->wind);
			} while ((rows * tf.FHeight) > global_df->SoundWin->winPixelSize) ;
