...
static void printTiers(char *sp, char *main, char *wor, char hasChanged) {
//	int bcnt;
	char delim[128];
	long beg, end;
...
static void segmentMainWorTiers(char *sp, char *main, char *wor, char *isJoinNext) {
	int m, w;
	...
	*isJoinNext = FALSE;
	...
	while (main[m] != EOS) {
		if (main[m] == '&' && main[m+1] == '&' && main[m+2] == '&') {
			...
			cw = wor[w];
			wor[w] = EOS;
			printTiers(sp, main, wor, hasChanged);
			main[m] = cm;
			strcpy(main, main+m);
			m = 0;
			wor[w] = cw;
...
void call() {
	...
	while (getwholeutter()) {
		...
		if (utterance->speaker[0] == '*') {
			if (isJoinNext == FALSE) {
				if (spareTier1[0] != EOS) {
					printTiers(sp, spareTier1, NULL, isJoinNext);
					spareTier1[0] = EOS;
				}
			} else if (!uS.partcmp(utterance->speaker, sp, FALSE, FALSE)) {
				strcat(spareTier1, " &&&");
				printTiers(sp, spareTier1, spareTier2, isJoinNext);
				isJoinNext = FALSE;
			}
			if (isJoinNext == FALSE) {
			...
			segmentMainWorTiers(sp, spareTier1, spareTier2, &isJoinNext);
			if (isJoinNext == FALSE)
				spareTier1[0] = EOS;
		} else {
			if (spareTier1[0] != EOS) {
