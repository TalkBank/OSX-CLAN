...
static char *DBGems[NUMGEMITEMS], *ke_script_file;
...
void init(char first) {
	...
	if (first) {
		...
		if (ftime) {
			...
			if (DBKeyRoot != NULL && strcmp(ke_script_file, "eng") && strcmp(ke_script_file, "zho") && strcmp(ke_script_file, "jpn")) {
			...
			if (isCreateDB) {
				if (strcmp(ke_script_file, "eng") && strcmp(ke_script_file, "zho") && strcmp(ke_script_file, "jpn")) {
					fprintf(stderr,"\nThe database can only be used with \"eng\", \"zho\" or \"jpn\" languages.\n");
					cutt_exit(0);
				}
