...
/*
	...
	Coverage:
	Rights:
	CMDI_Genre:	discourse
	CMDI_Interactivity:	interactive
	CMDI_PlanningType:	spontaneous
	CMDI_Involvement:	non-elicited
	CMDI_SocialContext:	family
	CMDI_EventStructure:	conversation
	CMDI_Channel:	face to face
	CMDI_Task:	unspecified
	CMDI_Modalities:	speech
	CMDI_Subject:	unspecified
	CMDI_EthnicGroup:	unspecified
	CMDI_RecordingConditions:	unspecified
	CMDI_AccessAvailability:	open access
	CMDI_Continent:	Europe
	CMDI_Country:	United Kingdom
	CMDI_WrittenResourceSubType:	SubType info
	CMDI_ProjectDescription:	some One Guy text
	CMDI_MediaFileDescription:	22050 Hz, 16 bit, 1 channel
*/
#define LANGNUM		10
#define LANGLEN		10
...
void usage() {
	printf("Usage: cmdi [cS dS l t] filename(s)\n");
	...
	puts("+t : just run test on data media consistency, this will not create *.cmdi files.");
	puts("\nExample:");
...
static void createCorpusCMDIFile(PATHTREE *p, char *wd_cur, METADATA *cmdata) {
	char *s;
	FILE *fp;
	PATHTREE *sib;
	...
	fprintf(fp, "<METATRANSCRIPT xmlns=\"http://www.mpi.nl/CMDI/Schema/CMDI\"\n");
	fprintf(fp, "                xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n");
	fprintf(fp, "                Date=\"%s\"\n", cur_date);
	fprintf(fp, "                FormatId=\"IMDI 3.03\"\n");
	fprintf(fp, "                Originator=\"cmdi.app\"\n");
	fprintf(fp, "                Type=\"CORPUS\"\n");
	fprintf(fp, "                Version=\"0\"\n");
	fprintf(fp, "                xsi:schemaLocation=\"http://www.mpi.nl/CMDI/Schema/CMDI ./CMDI_3.0.xsd\">\n");
	fprintf(fp, "  <Corpus>\n");
...
static void createSessionCMDIFile(PATHTREE *p, char *wd_cur, METADATA *cmdata) {
	int  i, j, cnt;
	long ln;
...
*/
		...
		if (iContinent != NULL && iCountry != NULL) {
			fprintf(fp, "        <Continent Link=\"http://www.mpi.nl/CMDI/Schema/Continents.xml\" Type=\"ClosedVocabulary\">%s</Continent>\n", iContinent->data);
			fprintf(fp, "        <Country Link=\"http://www.mpi.nl/CMDI/Schema/Countries.xml\" Type=\"ClosedVocabulary\">%s</Country>\n", iCountry->data);
...
//		fprintf(fp, "        <Key Name=\"Standard Template Key\"/>\n");
		...
		if (iGenre != NULL) {
			...
			strcpy(templineC2, "Discourse");
		fprintf(fp, "        <Genre Link=\"http://www.mpi.nl/CMDI/Schema/Content-Genre.xml\" Type=\"OpenVocabulary\">%s</Genre>\n", templineC2);
		fprintf(fp, "        <SubGenre Link=\"http://www.mpi.nl/CMDI/Schema/Content-SubGenre.xml\" Type=\"OpenVocabularyList\"/>\n");
		if (iTask != NULL)
			fprintf(fp, "        <Task Link=\"http://www.mpi.nl/CMDI/Schema/Content-Task.xml\" Type=\"OpenVocabulary\">%s</Task>\n", iTask->data);
		else
			fprintf(fp, "        <Task Link=\"http://www.mpi.nl/CMDI/Schema/Content-Task.xml\" Type=\"OpenVocabulary\">%s</Task>\n", "Unspecified");
		if (iModalities != NULL)
			fprintf(fp, "        <Modalities Link=\"http://www.mpi.nl/CMDI/Schema/Content-Modalities.xml\" Type=\"OpenVocabularyList\">%s</Modalities>\n", iModalities->data);
		else
			fprintf(fp, "        <Modalities Link=\"http://www.mpi.nl/CMDI/Schema/Content-Modalities.xml\" Type=\"OpenVocabularyList\">%s</Modalities>\n", "speech");
		if (iSubject != NULL)
			fprintf(fp, "        <Subject Link=\"http://www.mpi.nl/CMDI/Schema/Content-Subject.xml\" Type=\"OpenVocabularyList\">%s</Subject>\n", iSubject->data);
		else
			fprintf(fp, "        <Subject Link=\"http://www.mpi.nl/CMDI/Schema/Content-Subject.xml\" Type=\"OpenVocabularyList\">%s</Subject>\n", "Unspecified");
		fprintf(fp, "        <CommunicationContext>\n");
		if (iInteractivity != NULL)
			fprintf(fp, "          <Interactivity Link=\"http://www.mpi.nl/CMDI/Schema/Content-Interactivity.xml\" Type=\"ClosedVocabulary\">%s</Interactivity>\n", iInteractivity->data);
		else
			fprintf(fp, "          <Interactivity Link=\"http://www.mpi.nl/CMDI/Schema/Content-Interactivity.xml\" Type=\"ClosedVocabulary\">%s</Interactivity>\n", "interactive");
		if (iPlanningType != NULL)
			fprintf(fp, "          <PlanningType Link=\"http://www.mpi.nl/CMDI/Schema/Content-PlanningType.xml\" Type=\"ClosedVocabulary\">%s</PlanningType>\n", iPlanningType->data);
		else
			fprintf(fp, "          <PlanningType Link=\"http://www.mpi.nl/CMDI/Schema/Content-PlanningType.xml\" Type=\"ClosedVocabulary\">%s</PlanningType>\n", "spontaneous");
		if (iInvolvement != NULL)
			fprintf(fp, "          <Involvement Link=\"http://www.mpi.nl/CMDI/Schema/Content-Involvement.xml\" Type=\"ClosedVocabulary\">%s</Involvement>\n", iInvolvement->data);
		else
			fprintf(fp, "          <Involvement Link=\"http://www.mpi.nl/CMDI/Schema/Content-Involvement.xml\" Type=\"ClosedVocabulary\">%s</Involvement>\n", "non-elicited");
		if (iSocialContext != NULL)
			fprintf(fp, "          <SocialContext Link=\"http://www.mpi.nl/CMDI/Schema/Content-SocialContext.xml\" Type=\"ClosedVocabulary\">%s</SocialContext>\n", iSocialContext->data);
		else
			fprintf(fp, "          <SocialContext Link=\"http://www.mpi.nl/CMDI/Schema/Content-SocialContext.xml\" Type=\"ClosedVocabulary\">%s</SocialContext>\n", "Family");
		if (iEventStructure != NULL)
			fprintf(fp, "          <EventStructure Link=\"http://www.mpi.nl/CMDI/Schema/Content-EventStructure.xml\" Type=\"ClosedVocabulary\">%s</EventStructure>\n", iEventStructure->data);
		else
			fprintf(fp, "          <EventStructure Link=\"http://www.mpi.nl/CMDI/Schema/Content-EventStructure.xml\" Type=\"ClosedVocabulary\">%s</EventStructure>\n", "Conversation");
		if (iChannel != NULL)
			fprintf(fp, "          <Channel Link=\"http://www.mpi.nl/CMDI/Schema/Content-Channel.xml\" Type=\"ClosedVocabulary\">%s</Channel>\n", iChannel->data);
		else
			fprintf(fp, "          <Channel Link=\"http://www.mpi.nl/CMDI/Schema/Content-Channel.xml\" Type=\"ClosedVocabulary\">%s</Channel>\n", "Face to Face");
		fprintf(fp, "        </CommunicationContext>\n");
		...
		for (i=0; i < LANGNUM; i++) {
			if (flang[i][0] != EOS) {
				...
				fprintf(fp, "              <Name Link=\"http://www.mpi.nl/CMDI/Schema/MPI-Languages.xml\" Type=\"OpenVocabulary\">%s</Name>\n", templineC2);
				fprintf(fp, "              <Dominant Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</Dominant>\n", "Unspecified");
				fprintf(fp, "              <SourceLanguage Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</SourceLanguage>\n", "Unspecified");
				fprintf(fp, "              <TargetLanguage Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</TargetLanguage>\n", "Unspecified");
				fprintf(fp, "              <Description LanguageId=\"\" Link=\"\"/>\n");
				fprintf(fp, "            </Language>\n");
			}
		}
		...
		for (tID=rootIDs; tID != NULL; tID=tID->next_id) {
			fprintf(fp, "        <Actor>\n");
			fprintf(fp, "          <Role Link=\"http://www.mpi.nl/CMDI/Schema/Actor-Role.xml\" Type=\"OpenVocabularyList\"/>\n");
			if (tID->spname[0] == EOS)
			...
			if (tID->role[0] == EOS)
				fprintf(fp, "          <FamilySocialRole Link=\"http://www.mpi.nl/CMDI/Schema/Actor-FamilySocialRole.xml\" Type=\"OpenVocabularyList\"/>\n");
			else
				fprintf(fp, "          <FamilySocialRole Link=\"http://www.mpi.nl/CMDI/Schema/Actor-FamilySocialRole.xml\" Type=\"OpenVocabularyList\">%s</FamilySocialRole>\n", tID->role);
			fprintf(fp, "          <Languages>\n");
			fprintf(fp, "            <Description LanguageId=\"\" Link=\"\"/>\n");
			for (i=0; i < LANGNUM; i++) {
				if (tID->lang[i][0] != EOS) {
					...
					fprintf(fp, "              <Name Link=\"http://www.mpi.nl/CMDI/Schema/MPI-Languages.xml\" Type=\"OpenVocabulary\">%s</Name>\n", templineC2);
					if (tID->Llang[i])
						fprintf(fp, "              <MotherTongue Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</MotherTongue>\n", "true");
					else
						fprintf(fp, "              <MotherTongue Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</MotherTongue>\n", "Unspecified");
					fprintf(fp, "              <PrimaryLanguage Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">%s</PrimaryLanguage>\n", "Unspecified");
					fprintf(fp, "              <Description LanguageId=\"\" Link=\"\"/>\n");
					fprintf(fp, "            </Language>\n");
				}
			}
			...
			if (tID->sex == 0)
				fprintf(fp, "          <Sex Link=\"http://www.mpi.nl/CMDI/Schema/Actor-Sex.xml\" Type=\"ClosedVocabulary\">Unknown</Sex>");
			else if (tID->sex == 'm')
				fprintf(fp, "          <Sex Link=\"http://www.mpi.nl/CMDI/Schema/Actor-Sex.xml\" Type=\"ClosedVocabulary\">Male</Sex>\n");
			else
				fprintf(fp, "          <Sex Link=\"http://www.mpi.nl/CMDI/Schema/Actor-Sex.xml\" Type=\"ClosedVocabulary\">Female</Sex>\n");
			if (tID->education[0] == EOS)
				fprintf(fp, "          <Education>Unspecified</Education>\n");
			else
				fprintf(fp, "          <Education>%s</Education>\n", tID->education);
			fprintf(fp, "          <Anonymized Link=\"http://www.mpi.nl/CMDI/Schema/Boolean.xml\" Type=\"ClosedVocabulary\">true</Anonymized>\n");
			fprintf(fp, "          <Contact>\n");
			fprintf(fp, "            <Name/>\n");
			fprintf(fp, "            <Address/>\n");
			fprintf(fp, "            <Email/>\n");
	...
	if (!isJustTest && isMediaFound) {
		...
		fprintf(fp, "        <Type Link=\"http://www.mpi.nl/CMDI/Schema/MediaFile-Type.xml\" Type=\"ClosedVocabulary\">%s</Type>\n", ((isAudioFound) ? "audio" : "video"));
		fprintf(fp, "        <Format Link=\"http://www.mpi.nl/CMDI/Schema/MediaFile-Format.xml\" Type=\"OpenVocabulary\">%s</Format>\n", fformat);
		if (fsize[0] == EOS)
			fprintf(fp, "        <Size/>\n", fsize);
		else
...
/*
			...
			fprintf(fp, "        <Date>%s</Date>\n", fdate);
*/
		fprintf(fp, "        <Type Link=\"http://www.mpi.nl/CMDI/Schema/WrittenResource-Type.xml\" Type=\"OpenVocabulary\">Annotation</Type>\n");
		if (iWrittenSubtype != NULL)
			fprintf(fp, "        <SubType Link=\"http://www.mpi.nl/CMDI/Schema/WrittenResource-SubType.xml\" Type=\"OpenVocabularyList\">%s</SubType>\n", iWrittenSubtype->data);
		else
			fprintf(fp, "        <SubType Link=\"http://www.mpi.nl/CMDI/Schema/WrittenResource-SubType.xml\" Type=\"OpenVocabularyList\"/>\n");
		fprintf(fp, "        <Format Link=\"http://www.mpi.nl/CMDI/Schema/WrittenResource-Format.xml\" Type=\"OpenVocabulary\">%s</Format>\n", "text/x-chat");
		if (fsize[0] == EOS)
			fprintf(fp, "        <Size/>\n", fsize);
		else
			fprintf(fp, "        <Size>%s</Size>\n", fsize);
		fprintf(fp, "        <Validation>\n");
		fprintf(fp, "          <Type Link=\"http://www.mpi.nl/CMDI/Schema/Validation-Type.xml\" Type=\"ClosedVocabulary\">%s</Type>\n", "Formal/Content");
		fprintf(fp, "          <Methodology Link=\"http://www.mpi.nl/CMDI/Schema/Validation-Methodology.xml\" Type=\"ClosedVocabulary\">%s</Methodology>\n", "Automatic");
		fprintf(fp, "          <Level>100</Level>\n");
		fprintf(fp, "          <Description LanguageId=\"\" Link=\"\"/>\n");
		fprintf(fp, "        </Validation>\n");
		fprintf(fp, "        <Derivation Link=\"http://www.mpi.nl/CMDI/Schema/WrittenResource-Derivation.xml\" Type=\"ClosedVocabulary\">%s</Derivation>\n", "Annotation");
	...
	if (p != NULL) {
		...
		if (p->child != NULL) {
			if (p->name != NULL) {
				...
				if (ldata != NULL)
					createCorpusCMDIFile(p, wd_cur, ldata);
				else
					createCorpusCMDIFile(p, wd_cur, cmdata);
				if (ldata != NULL)
					free_metadata(ldata);
			}
			writingTree(p->child, wd_cur, cmdata);
		} else if (p->name != NULL) {
			createSessionCMDIFile(p, wd_cur, cmdata);
		}
		sib = p->sibling;
		while (sib != NULL) {
			wd_cur[len] = EOS;
			if (sib->child != NULL) {
				if (sib->name != NULL) {
					...
					if (ldata != NULL)
						createCorpusCMDIFile(sib, wd_cur, ldata);
					else
						createCorpusCMDIFile(sib, wd_cur, cmdata);
					if (ldata != NULL)
						free_metadata(ldata);
				}
				writingTree(sib->child, wd_cur, cmdata);
			} else if (sib->name != NULL) {
				createSessionCMDIFile(sib, wd_cur, cmdata);
			}
			sib = sib->sibling;
		}
		wd_cur[len] = EOS;
